...
#create dataset for LTM oricessing sequences of length AR
def create_lstm_dataset(x_train,AR=10):
    data=[]
    for i in range (len(x_train)-AR-1):
        data.append(x_train[i:(i+AR)])
    return np.array(data)

#define the RNN Model
def create_lstm_model(layers):
    model = Sequential()
...
def load_last_chekpoint(checkpoint_path):
    if not os.listdir(checkpoint_path) :
        return
    filefilter = r'\*.h5'
    files = glob.glob(checkpoint_path+filefilter)
...
def main():
    ...
    pt = PowerTransformer()
    data_tr=pt.fit_transform(data_tr)
    ...
    #prepare lstm data
    AR=10
    data_tr = create_lstm_dataset(data_tr,AR)
    label_tr = label_tr[AR+1:]
    
    # Create Regressor Model
    model_reg = create_lstm_model([14,AR])
    print(model_reg.summary())

    print('Creating callbacks..')
    tb_callback = tf.keras.callbacks.TensorBoard(log_dir="logs/",histogram_freq=1)
    checkpoint_filename = 'weights.{epoch:02d}-{val_loss:.2f}.h5'
    checkpoint_cb = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_filename,
                                                        monitor='loss',
                                                        mode='min',
                                                        save_best_only=True)
    earlystopping_cb = tf.keras.callbacks.EarlyStopping(monitor='loss',
                                                         min_delta=0,
                                                         patience=10,
                                                         verbose=2,
                                                         mode='auto',
...
#    for e in range(1,epochs):
    checkpoint_filepath='.'
    load_saved =True
    last_saved_model = load_last_chekpoint(checkpoint_filepath)
    if load_saved == True and last_saved_model is not None:
        print('Loading last saved model {}'.format(last_saved_model))
        model_reg = load_model(os.path.join(checkpoint_filepath,last_saved_model))
        
    print('.'),
    history = model_reg.fit(data_tr,label_tr,batch_size=32,
                            epochs=epochs,validation_split=0.3,
                            verbose=1,callbacks=[checkpoint_cb,earlystopping_cb,tb_callback])
    print('.')
    ...
    if not predict_all :
        ...
        data_tr=pt.fit_transform(data_ts)
        data_ts = create_lstm_dataset(data_ts)
        print('Shape of test{}'.format(data_ts.shape))
        label_prd = model_reg.predict(data_ts)
        print('Pridiction for Test Motor {}'.format(test_motor),label_prd.shape)
        print(label_ts)
        print(label_prd)
        print('test RMSE:',np.sqrt(metrics.mean_squared_error(label_ts[AR+1:], label_prd)))
        print( 'Error Analysis :')
        print_error_analysis(label_ts[AR+1:], label_prd)
